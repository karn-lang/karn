...
Val fib(Val n) {
    Val _subj = n;
    if (_subj.type == VAL_NUM && _subj.as.num == 0) { return val_num(0); }
    if (_subj.type == VAL_NUM && _subj.as.num == 1) { return val_num(1); }
    { Val x = _subj; return val_num(_val_to_num(fib(val_num(_val_to_num(x) - _val_to_num(val_num(1))))) + _val_to_num(fib(val_num(_val_to_num(x) - _val_to_num(val_num(2)))))); }
    return val_nil();
}

int main(void) {
    srand(time(NULL));
    val_println(fib(val_num(10)));
    return 0;
}