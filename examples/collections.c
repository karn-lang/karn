...
Val _fn1(Val x) {
    return val_num(_val_to_num(x) * _val_to_num(val_num(2)));
}

Val _fn2(Val x) {
    return val_bool(_val_to_num(val_num(fmod(_val_to_num(x), _val_to_num(val_num(2))))) == _val_to_num(val_num(0)));
}

Val _fn3(Val x) {
    return val_bool(_val_to_num(x) > _val_to_num(val_num(10)));
}

Val _fn4(Val x) {
    return val_num(_val_to_num(x) * _val_to_num(val_num(3)));
}

int main(void) {
    srand(time(NULL));
    Val items = ({ Arr* _a=arr_new(10); arr_push(_a, val_num(1)); arr_push(_a, val_num(2)); arr_push(_a, val_num(3)); arr_push(_a, val_num(4)); arr_push(_a, val_num(5)); arr_push(_a, val_num(6)); arr_push(_a, val_num(7)); arr_push(_a, val_num(8)); arr_push(_a, val_num(9)); arr_push(_a, val_num(10)); val_arr(_a); });
    Val doubled = ({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) arr_push(_a, _fn1(items.as.arr->items[_i])); val_arr(_a); });
    Val evens = ({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) if(_val_to_num(_fn2(items.as.arr->items[_i]))) arr_push(_a, items.as.arr->items[_i]); val_arr(_a); });
    Val result = ({ Arr* _a=arr_new(({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) arr_push(_a, _fn4(items.as.arr->items[_i])); val_arr(_a); }).as.arr->len); for(int _i=0;_i<({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) arr_push(_a, _fn4(items.as.arr->items[_i])); val_arr(_a); }).as.arr->len;_i++) if(_val_to_num(_fn3(({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) arr_push(_a, _fn4(items.as.arr->items[_i])); val_arr(_a); }).as.arr->items[_i]))) arr_push(_a, ({ Arr* _a=arr_new(items.as.arr->len); for(int _i=0;_i<items.as.arr->len;_i++) arr_push(_a, _fn4(items.as.arr->items[_i])); val_arr(_a); }).as.arr->items[_i]); val_arr(_a); });
    Val one_to_hundred = karn_range(val_num(1), val_num(100));
    Val _subj = val_ok(val_num(42));
    if (_subj.type == VAL_OK) { Val v = *_subj.as.ok_val; val_println(v); }
    if (_subj.type == VAL_ERR) { Val e = _subj; val_println(val_num(-1)); }
    return 0;
}